...
### Antigravity: high-resolution transcripts

This applies to both Antigravity variants: the IDE (`antigravity`) and the CLI
(`antigravity-cli`). The IDE stores each session as a SQLite `.db` file; the CLI
appears in two on-disk formats -- newer releases store conversation trajectories
as SQLite `.db` files, older releases used AES-GCM-encrypted `.pb` files. In
every case, the full transcript -- structured tool calls, results, reasoning,
and diffs -- comes from a `<uuid>.trajectory.json` sidecar. Without a covering
sidecar, agentsview falls back to **summary mode**: a heuristic decode of the
raw `.db` steps (prompts and tool-call names only), or for `.pb` sessions your
prompts from `history.jsonl` plus any plain-text artifacts under `brain/`
(plans, walkthroughs, checkpoints). Summary-mode sessions show a "Summary mode"
badge in the detail header that links here.
